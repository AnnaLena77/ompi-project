...
#else
      res = NBC_Sched_recv (recvbuf, false, recvcount, datatype, 0, schedule, false, &item);
#endif
      if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        OBJ_RELEASE(schedule);
        free(tmpbuf);
...
#else
        res = NBC_Sched_send (sbuf, true, recvcount, datatype, r, schedule, false, &item);
#endif
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
          OBJ_RELEASE(schedule);
          free(tmpbuf);
...
#else
  res = NBC_Sched_send (sendbuf, false, count, dtype, 0, schedule, false, &item);
#endif
  if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
    OBJ_RELEASE(schedule);
    free(tmpbuf);
...
#else
    res = NBC_Sched_recv (lbuf, true, count, dtype, 0, schedule, true, &item);
#endif
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
      OBJ_RELEASE(schedule);
      free(tmpbuf);
...
#else
      res = NBC_Sched_recv (rbuf, true, count, dtype, peer, schedule, true, &item);
#endif
      if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        OBJ_RELEASE(schedule);
        free(tmpbuf);
...
#else
      res = NBC_Sched_local_send (lbuf + ext * rcount * peer, true, rcount, dtype, peer, schedule, false, &item);
#endif
      if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        OBJ_RELEASE(schedule);
        free(tmpbuf);
...
#else
    res = NBC_Sched_local_recv(recvbuf, false, rcount, dtype, 0, schedule, false, &item);
#endif
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
      OBJ_RELEASE(schedule);
      free(tmpbuf);
