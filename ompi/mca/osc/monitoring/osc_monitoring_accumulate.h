...
#define OSC_MONITORING_GENERATE_TEMPLATE_ACCUMULATE(template)           \
    ...
    {                                                                   \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_compare_and_swap(origin_addr, compare_addr, result_addr, dt, target_rank, target_disp, win, NULL); \
    }                                                                   \
                                                                        \
    static int ompi_osc_monitoring_## template ##_get_accumulate (const void *origin_addr, \
                                                                  int origin_count, \
                                                                  ...
                                                                  int target_count, \
                                                                  ompi_datatype_t*target_datatype, \
                                                                  ompi_op_t *op, ompi_win_t*win, qentry **q) \
    {                                                                   \
        int world_rank;                                                 \
        /**                                                             \
         * If this fails the destination is not part of my MPI_COM_WORLD \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_get_accumulate(origin_addr, origin_count, origin_datatype, result_addr, result_count, result_datatype, target_rank, target_disp, target_count, target_datatype, op, win, NULL); \
    }                                                                   \
                                                                        \
    static int ompi_osc_monitoring_## template ##_rget_accumulate (const void *origin_addr, \
                                                                   int origin_count, \
    ...
    {                                                                   \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_rget_accumulate(origin_addr, origin_count, origin_datatype, result_addr, result_count, result_datatype, target_rank, target_disp, target_count, target_datatype, op, win, request, NULL); \
    }                                                                   \
                                                                        \
    static int ompi_osc_monitoring_## template ##_raccumulate (const void *origin_addr, \
                                                               int origin_count, \
    ...
    {                                                                   \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_raccumulate(origin_addr, origin_count, origin_datatype, target_rank, target_disp, target_count, target_datatype, op, win, request, NULL); \
    }                                                                   \
                                                                        \
    static int ompi_osc_monitoring_## template ##_accumulate (const void *origin_addr, \
                                                              int origin_count, \
    ...
    {                                                                   \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_accumulate(origin_addr, origin_count, origin_datatype, target_rank, target_disp, target_count, target_datatype, op, win, NULL); \
    }                                                                   \
                                                                        \
    static int ompi_osc_monitoring_## template ##_fetch_and_op (const void *origin_addr, \
                                                                void *result_addr, \
    ...
    {                                                                   \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_fetch_and_op(origin_addr, result_addr, dt, target_rank, target_disp, op, win, NULL); \
    }
