...
void initQentry(qentry **q){
    if(q==NULL || *q==NULL){
        ...
        qentry *item = *q;
        item->id = 0;
        memcpy(item->function, "", 1);
        item->blocking = -1;
        memcpy(item->datatype, "", 1);
        item->count = 0;
        item->sendcount = 0;
        item->recvcount = 0;
        item->datasize = 0;
        memcpy(item->operation, "", 1);
        memcpy(item->communicationArea, "", 1);
        memcpy(item->processorname, "", 1);
        item->processrank = -1;
        item->partnerrank = -1;
        memcpy(item->sendmode, "", 1);
        item->immediate = 0;
        memcpy(item->usedBtl, "", 1);
        memcpy(item->usedProtocol, "", 1);
        item->withinEagerLimit = -1;
        item->foundMatchWild = -1;
        memcpy(item->usedAlgorithm, "", 1);
